...
// a person with a name and age property, write a function 
// that returns a new array containing the names of all people
...
function getNamesOfAdults(people){
    let adults = []
     let newArray =
     people.filter(person => person.age >=18).map(adult => adult.name);
    adults.push(newArray)
    console.log(adults)
return adults
}
...
//  a product  with a name, price, and category property. The function should
// return an object that groups the products by their categories, 
...
function groupProductsByCategory(products) {
        ...
        let groupByCategories = Object.groupBy(products, product =>{
...
//  students whose average score is greater than or equal to 85.
  ...
  function averageScore(){
    ...
    students.forEach(i => {
     let sum = i.scores.reduce((x,y) => x +y,0);
     let average = sum/i.scores.length
     if(average >= 85){
         newArr.push(i.name)
     }});
